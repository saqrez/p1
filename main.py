...
from bokeh.models import ColumnDataSource,CDSView,DataTable,TableColumn,NumberFormatter
...
product = Select(title="Product",  options=sorted(prod), value="All")

city = []
city.append('All')

city.extend(data_ols['city'].unique().tolist())
city = Select(title="city",  options=sorted(city), value="All")

action=['None','Forecast']
act=Select(title="Action",options=action, value="None")
...
f1=figure(x_axis_type='datetime',tooltips=TOOLTIPS, title="Forecast",plot_width=900, plot_height=250)
p = figure(x_axis_type='datetime',tooltips=TOOLTIPS,title="Data Exploration",plot_width=900, plot_height=300)	
...
def update_plot(attr, old, new):
    ...
    p2 = figure(x_axis_type='datetime',tooltips=TOOLTIPS,plot_width=900, plot_height=250)
    p2.extra_y_ranges['disc'] = Range1d(start=0, end=1)
    p2.add_layout(LinearAxis(y_range_name='disc', axis_label='Discount%'), 'right')
    p2.xaxis.formatter.days = '%m/%d/%Y'
    p2.xaxis.axis_label = 'Date'
    # p2.background_fill_color = "black"
    # p2.xgrid.visible = False
    # p2.ygrid.visible = False
    # p2.yaxis.axis_label = 'Quantity'
    
    # p2.add_layout(LinearAxis(y_range_name="Footfall"), 'right')
    layout.children[0] = column(row([product,city], width=400), p2)
    # layout2 = row(column(row([product,city],width=400),p2),column(column([bt],width=300),f1))
    # layout.children[1] = p2
    f2 = figure(x_axis_type='datetime',tooltips=TOOLTIPS,plot_width=900, plot_height=250)
    ...
    if city.value!= "All":
        ...
        newSource =  ColumnDataSource(df3)
    ...
    if act.value!="None":
        ...
        data_table = DataTable(source=tabsource, columns=columns, width=400, height=250, index_position=None,editable=True,fit_columns=True)
        # layout.children[2] = column(data_table)
        # curdoc().add_root(data_table)
        # layout.children[1] = column(row([act], width=400), row(f2))
        layout.children[0] = column(row([product,city], width=400), row(p2,data_table))
        # print("working")
    #     traindata=pd.merge(train,df3, how="left",left_on="date",right_on="date_fld")
    ...
    #     traindata['tot_qty']=traindata['tot_qty'].fillna(0)
          
    #     traindata=traindata.fillna(method="ffill")
    #     traindata=traindata.fillna(method="bfill")
    #     traindata['date_fld']=traindata['date']
        newSource2 =  ColumnDataSource(fd)
        plotf=f2.line(x='date_fld', y='y', source=newSource2, line_width=3,line_color='red')
        plotf=f2.line(x='date_fld', y='pred0.05', source=newSource2, line_width=3,line_color='green')
        f2.add_tools(HoverTool(renderers=[plotf], tooltips=TOOLTIPS,
    ...
    # plot1=p2.line(x='date_fld', y='tot_qty', source=newSource, line_width=3)
    # 
    # plotf=f2.line(x='date_fld', y='pred0.0', source=newSource2, line_width=3,line_color='red')
            
    plot1=p2.circle(x='date_fld', y='tot_qty', source=newSource, size=10,fill_color="green")
    plot1=p2.circle(x='date_fld', y='disc_pct', source=newSource,size=10)
    # plot2=p2.circle(x='date_fld', y='disc_pct', source=newSource,fill_color="red",size=10,y_range_name='disc')
    p2.add_tools(HoverTool(renderers=[plot1], tooltips=TOOLTIPS,
                           formatters = {"@date_fld": "datetime","@disc_pct":"numeral"},mode='vline'))
...
city.on_change('value', update_plot)
act.on_change('value', update_plot)
