...
def forecast(data,mcal):
    ...
    def forecast(attr, old, new):
        ...
        f2 = figure(x_axis_type='datetime',title="Forecast",tooltips=tt_categ,plot_width=1200, plot_height=300)
        ...
        if product.value!= "Select":
            if city.value!= "Select":
                if act.value!="None":
                    ...
                    newSource3= ColumnDataSource(fd2)
                    combined_fd=fd.append([fd2])
                    # methodlist=['LGBM','OLS']
                    # xs = []
                    # ys = []
                    # colors = []
                    ...
                    # for i, prd in enumerate(methodlist):
                    #     lbl=prd
                    # # for i, prd in enumerate(prod_list):
                    #     subset = combined_fd[combined_fd['product_desc'] == prd]
                    #     x = subset['date_fld']
                    #     y= subset['tot_qty']
                    #     xs.append(list(x))
                    #     ys.append(list(y))
                    ...
                    plotf=f2.line(x='date_fld', y='y', source=fd, line_width=2,line_color='white',legend_label="Actual")
                    plotf=f2.line(x='date_fld', y=y, source=fd, line_width=2,line_color='green',legend_label="OLS")
                    plotf=f2.line(x='date_fld', y='gbm', source=fd, line_width=2,line_color='yellow',legend_label="LGBM")
    ...
    def ols_act (data_ols):
        ...
        for pct in (0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4):
            ...
            futuredf2 = pd.merge(futuredf2,traindata[['ds','y','zone_qty','year']],on='ds', how='left')
        ...
        for pct in (0.0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4):
            ...
            future = pd.merge(futuredf,traindata[['ds','y','zone_qty','year']],on='ds', how='left')
                # future['y_lag1y']=future['y'].shift(364)
